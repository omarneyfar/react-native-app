...
const signup = () => {
  ...
  const handleSignUp = async () => {
    try {
      ...
      const { data } = await service.signUp.signUpCreate(credentials);
      if (data.accessToken) {
        router.push("/login");
      } else {
  ...
  return (
    <View>
      ...
      <View style={styles.providerWrapper}>
        {providers.map &&
          providers.map((provider: Provider, index: number) => (
            <ButtonProvider key={index} provider={provider} />
          ))}
      </View>
      <DividerText title="or" />
      <View style={styles.formWrapper}>
        ...
        <ButtonSubmit title="Create account" onClick={handleSignUp} />
      </View>
